...
def test_start_train(mock_load_hyperparams, mock_create_ddpg_agent, mock_create_simulation_env, confidence_instance):
    ...
    mock_load_hyperparams.return_value = ({}, {})  # Return empty dicts for params and hidden
    ...
    with patch.object(confidence_instance, 'train', return_value=(None, [], [], {}, {}, {}, None)) as mock_train, \
         patch.object(confidence_instance, 'evaluate_agent', return_value=100) as mock_evaluate, \
         patch('your_script.save_all'), \
         patch('os.makedirs'), \
         patch('os.getcwd', return_value='/fakepath'):
        confidence_instance.start_train('fake_config.yml', 'fake_eval_config.yml', 'fake_param_file.yml')
...
def test_evaluate_agent(confidence_instance):
    ...
    mock_agent = MagicMock()
    mock_agent.actor.return_value = MagicMock(detach=MagicMock(return_value=[0]))  # Simplistic action
