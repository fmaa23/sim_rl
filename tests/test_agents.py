...
def compare_state_dicts(state_dict1, state_dict2):
    assert isinstance(
        state_dict1, OrderedDict), "state_dict1 is not an OrderedDict."
    assert isinstance(
        state_dict2, OrderedDict), "state_dict2 is not an OrderedDict."
    for (k1, v1), (k2, v2) in zip(state_dict1.items(), state_dict2.items()):
        ...
        # print(v2)
        assert torch.equal(
            v1, v2), "The values of the state_dict are different."
...
def test_fit_model(ddpg_agent):
    ...
    with patch('torch.optim.Adam.step') as mock_step:
        ...
        assert len(
            initial_loss_reward) > 0, "No reward model training loss was recorded."
        assert len(
            initial_loss_next_state) > 0, "No next state model training loss was recorded."
...
def test_update_q_values(ddpg_agent):
    ...
    batch = [(torch.randn(n_states), torch.randn(n_actions),
              torch.randn(1), torch.randn(n_states)) for _ in range(5)]
...
def test_update_critic_network(ddpg_agent, num_experiences=100):
    ...
    for experience in batch:
        ...
        next_q_value = baseline_critic_target(
            [next_state, baseline_actor_target(next_state).detach()]).detach()
