...
def num_runs():
    ...
    return NumRuns(confidence_level=0.95, desired_error=1, num_runs=10, time_steps=100, num_sim=100)
...
def test_get_std(num_runs):
    ...
    with patch.object(num_runs, 'train_multi_agents', return_value=[{'key': {'inner_key': np.random.normal(0, 1, 100)}} for _ in range(10)]):
