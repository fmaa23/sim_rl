...
def test_load_json_data(mock_load, mock_getcwd, mock_normpath, mock_file):
    ...
    mock_normpath.return_value = "/fake/directory/foundations/output_csv/reward_dict.json"
    app = startup_behavior(5, 0.01, 3, 0)
    data = app.load_json_data()
    assert data == sample_data
    mock_file.assert_called_once_with("/fake/directory/foundations/output_csv/reward_dict.json", 'r')
...
@pytest.mark.parametrize("data, expected", [
    ([0.02, 0.01, 0.01, 0.01, 0.01], -1),
    ([0.02, 0.2, 0.1, 0.05, 0.02], -1)
])
