...
def train(params, agent, env, best_params = None):
    ...
    save_state = {'state':[]}
    env.reset()
    state = env.explore_state(agent, env.qn_net, episode = 0)
    state_list = state.tolist()
    state_int = [int(x) for x in state_list]
    initial_states = convert_format(state_int)
    env.net.set_initial_states(initial_states)
    print("Initial State to start the simulation:")
    print()
    print(f"start state from explore_state: {state_int}")
    print(f"start state from env.net.num_agents: {env.net.num_agents}")
    print()
    for episode in range(num_episodes): # tqdm(range(num_episodes), desc="Episode Progress"): 
        print(f"after the {episode+1}th 100 simulations:")
        print()
        print(f"we start from state (get from env.net.num_agents): {env.net.num_agents}")
        # state_list = state.tolist()
        # state_int = [int(x) for x in state_list]
        # initial_states = convert_format(state_int)
        #env.net.set_initial_states(initial_states)
        ...
        print(f"next state from env.get_next_state: {next_state}")
        print(f"next state from env.net.num_agents: {env.net.num_agents}")
        print()
